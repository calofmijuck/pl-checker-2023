...
module TestEx2: TestEx =
  struct
    ...
    let testcases =
      ...
      ; CALC (SIGMA(INT 1, INT 10, SIGMA(X, INT 1, INT 10)), FLOAT 10.0)
      ; CALC (SIGMA(INT 1, INT 1, SIGMA(X, INT 1, X)), FLOAT 1.0)
      ; CALC (SIGMA(INT 1, INT 2, SIGMA(INT 3, X, X)), FLOAT 0.0)
      ; CALC (SIGMA(INT 1, INT 2, SIGMA(X, ADD(X, INT 1), SIGMA(X, X, INT 1))), FLOAT 4.0)
      ; CALC (INTEGRAL(REAL 1.0, REAL 2.0, SIGMA(X, INT 10, REAL (-19.2))), FLOAT (-172.8))
      ; CALC (INTEGRAL(INT 1, INT 2, SIGMA(X, INT 5, MUL(X, X))), FLOAT 49.5)
      ; CALC (SIGMA(INT 1, INT 10, INTEGRAL(X, ADD(X, INT 1), MUL(X, X))), FLOAT 434.627)
      ; CALC (INTEGRAL(REAL 0.0, REAL 1.0, INTEGRAL(REAL 0.0, MUL(X, X), SUB(X, REAL 2.0))), FLOAT (-0.427))
      ]
